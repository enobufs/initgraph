...
class Mod {
    ...
    initialize() {
        ...
        if (this._state === State.UNINITING) {
            return new Promise.reject(new Error('initgraph: already initializing'));
        }
        ...
        .catch((err) => {
            this._state = State.CLOSED;
            err.message = `initgraph: failed to initialize ${this.name}: ${err.message}`;
            throw err;
        })
        .then(() => {
    ...
    uninitialize() {
        ...
        if (this._state === State.INITING) {
            return new Promise.reject(new Error('cannot uninitialize while initializing'));
        }
